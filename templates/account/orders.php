...
<div class="alert alert--warning">Bạn không có đơn hàng nào.
    <a
        href="<?= get_post_type_archive_link('product'); ?>"><?php esc_html_e('Click vào đây ', 'gtt-shop') ?></a><?php esc_html_e('để bắt đầu mua sản phẩm', 'gtt-shop') ?>
</div>
<?php
	return;
endif;
...
<table class="orders">
    ...
    <tr>
        ...
        <td><a
                href="<?= get_permalink(ps_setting('confirmation_page')) ?>?view=order&id=<?= $item->id; ?>"><?php esc_html_e('Xem chi tiết', 'gtt-shop') ?></a>
        </td>
    </tr>
    <?php endforeach; ?>
</table>